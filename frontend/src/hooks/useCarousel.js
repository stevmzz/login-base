...
export function useCarousel(images, interval) {
  ...
  const goToSlide = (index) => setCurrentIndex(index); // Go to a specific slide
  const nextSlide = () =>
    setCurrentIndex((prevIndex) => (prevIndex + 1) % images.length); // Go to the next slide
  const prevSlide = () =>
    setCurrentIndex((prevIndex) => (prevIndex - 1 + images.length) % images.length); // Go to the previous slide
